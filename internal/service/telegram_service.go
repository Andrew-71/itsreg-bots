...
func (s *TelegramService) Status(_ context.Context, uuid string) (bots.Status, error) {
	r, ok := s.m.Load(uuid)
	if !ok {
		return bots.Stopped, nil
	}

	ins := r.(*botInstance)
	if ins.IsDead() {
		return bots.Failed, nil
	}
	return bots.Started, nil

}

func (s *TelegramService) Start(ctx context.Context, id string, token string) error {
	_, ok := s.m.Load(id)
	if ok {
...
func (s *TelegramService) Stop(_ context.Context, id string) error {
	r, ok := s.m.Load(id)
	ins := r.(*botInstance)
	if !ok {
...
func (i *botInstance) handleUpdate(ctx context.Context, upd tgbotapi.Update) {
	...
	if upd.Message != nil {
		if upd.Message.IsCommand() {
			err = i.entry.Entry(ctx, i.botId, upd.Message.Chat.ID, upd.Message.Command())
		} else {
			// note: is this Must dangerous?
			err = i.process.Process(ctx, i.botId, upd.Message.Chat.ID, bots.MustNewPlainMessage(upd.Message.Text))
		}
	}
